...
def socket_send_command_and_recive_all_data(socket_p, command):
    if not command:
        return None
...
while True:
    infos_data = socket_send_command_and_recive_all_data(connection_socket, "infos")
    if not infos_data:
        break
    ...
    data_recues = socket_send_command_and_recive_all_data(connection_socket, commande)
    if not data_recues:
        break

    if dl_filename: 
        if len(data_recues) == 1 and data_recues == b" ":
             print("Erreur: Ce fichier", dl_filename ,"n'existe pas.")
        else: 
